if (typeof window.linkedInJobsAssistInjected === "undefined") {
  ...
  function detectChallenge() {
    ...
    const text = String(document.body?.innerText || "").substring(0, 3000);
    return /security verification|unusual activity|captcha|verificacao de seguranca/i.test(
      text,
    );
  }
  ...
  function extractJobFromCard(card, idx) {
    ...
    const jobUrl = linkEl ? linkEl.href.split("?")[0] : "";
    ...
    return {
      ...
      alreadyApplied: /applied|candidatura enviada|ja se candidatou/.test(
        cardText,
      ),
      postedHoursAgo,
      seniority: title,
  ...
  function needsJobDetailContext(config) {
    return (
      config?.jobsBrazilOffshoreFriendly === true ||
      (Array.isArray(config?.keywordTerms) && config.keywordTerms.length > 0)
    );
  }
  ...
  function fillKnownFields(modal, profile) {
    ...
    for (const field of fields) {
      ...
      if (
        (/email/.test(hint) || field.type === "email") &&
      ...
      if (
        (/phone|telefone|celular/.test(hint) || field.type === "tel") &&
      ...
      if (
        /full name|nome completo|your name|first and last name|nome e sobrenome/.test(
          hint,
        ) &&
        !/company|school|employer|organization|reference|empresa|escola/.test(
          hint,
        ) &&
      ...
      if (
        /headline|current title|cargo atual|title/.test(hint) &&
      ...
      if (
        /portfolio|website|site/.test(hint) &&
      ...
      if (
        /city|location|cidade|localizacao/.test(hint) &&
      ...
      if (
        /summary|about|resumo|cover letter/.test(hint) &&
  ...
  function findEasyApplyButton() {
    ...
    for (const btn of buttons) {
      ...
      if (
        /easy apply|candidatura simplificada/.test(text) ||
  ...
  async function waitForModalStepChange(
    previousModal,
    previousSignature,
    options,
  ) {
  ...
  async function runModalStepFlow(initialModal, profile, options) {
    ...
    while (stepCount < options.maxModalSteps) {
      ...
      if (reviewBtn) {
        const signature = getModalSignature(modal);
        reviewBtn.click();
        const moved = await waitForModalStepChange(modal, signature, options);
        stepCount++;
        if (!moved.changed) {
  ...
  async function prepareJobForManualReview(job, profile, runtimeOptions) {
    ...
    const result = await runModalStepFlow(ready.modal, profile, options);
    if (
      result &&
  ...
  async function runJobsAssist(config, runtimeOptions) {
    ...
    const ranked =
      typeof rankJobsForApply === "function"
    ...
    for (const job of ranked.slice(0, rankedLimit)) {
      ...
      processed++;
      if (
        prepared.status.startsWith("skipped") ||
    ...
    return {
      ...
      reason: processed > 0 ? "unknown" : "no-items-processed",
      processedCount: processed,
      actionCount: Math.max(0, processed - skipped),
      skippedCount: skipped,
      log: jobsLog,
  ...
  window.addEventListener("message", async (event) => {
    ...
    try {
      ...
      const templateMeta = event.data.config?.templateMeta;
      if (
        runtimeResult &&
      ...
      window.postMessage(
        {
          ...
          result: {
            ...
            actionCount: jobsLog.filter(
              (entry) => !/^skipped|^skip-/.test(String(entry?.status || "")),
            ).length,
            skippedCount: jobsLog.filter((entry) =>
              /^skipped|^skip-/.test(String(entry?.status || "")),
            ).length,
            log: jobsLog,
            templateMeta: event.data.config?.templateMeta,
          },
        },
        "*",
      );
