...
- **Companies follow confirmation retries** — each follow click is verified in-card with bounded retries and signal polling (`Following`/`Seguindo`, aria followed state, follow-success toast); unconfirmed attempts are logged as `skipped-follow-not-confirmed` and zero-follow runs can fail with `follow-not-confirmed`
...
- **Jobs mode may stop with `manual-input-required`** — complete missing required answers in the open Easy Apply modal and restart if needed
